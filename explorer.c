...
void* exit_null(void* v,const char* msg) {
    if (!v) {endwin();perror(msg);exit(EXIT_FAILURE);}
    return v;
}
const char* menu_str(int errnum) {
    switch (errnum) {
        ...
        case E_NOT_CONNECTED:return "No items are connected to the menu.";
        case E_BAD_ARGUMENT:return "Routine detected an incorrect or out-of-range argument.";
        case E_POSTED:return "The menu has already been posted.";
        case E_CONNECTED:return "Item is connected to a menu.";
        default:return "Unknown menu_errnum";
    }
}
void err_menu(int v,const char* msg){
    if (v!=E_OK) {
        endwin();fprintf(stderr,"%s: %s",msg,menu_str(v));fflush(stderr);exit(EXIT_FAILURE);
    }
}
void* err_ptr(void* v,const char* msg){
    if (!v) {
        endwin();fprintf(stderr,"%s: %s",msg,menu_str(errno));fflush(stderr);exit(EXIT_FAILURE);
    }
    return v;
}
...
void free_items(ITEM** items,size_t items_len, char** strings,size_t strings_len, int* item_i) {
    for (size_t i=0;i<items_len&&i<(size_t)*item_i;i++) {
        if (!items[i]){fprintf(stderr,"ERROR:(item_len{%zu}!=actual len{%zu}, item_i:%d)items[%zu]==NULL(%p)\n",items_len,i,*item_i,i,(void*)items[i]);break;}
...
void add_items(struct jobject* curr, ITEM*** items_array, size_t* items_len, char*** strings_array, size_t* strings_len, int* item_i) {
    char* buf,*src;
    ...
    switch (curr->type) {
        ...
        case JNUMBER:case JBOOL:case JNULL:case UNKNOWN:;
            ...
            buf = exit_null(sprint_object(curr,buf,&offset,&buf_len),"sprint_object");
...
void add_type(enum type type,ITEM*** items, size_t* items_len, char*** strings, size_t* strings_len, int* item_i) {
...
struct jobject* append_level(struct jobject*** levels, size_t* levels_len, int* level, struct jobject* val) {
    struct jobject** buf=*levels;
    if ((size_t)*level>=*levels_len) {
        buf = realloc(*levels, (*level+1)*sizeof(struct jobject*));
...
void redraw(WINDOW* wm,MENU* m, struct jobject* curr, int* item_i, ITEM*** items,size_t* items_len, char*** strings,size_t* strings_len, int level) {
    //clear da menu
...
int main(int argc, char **argv) {
    if (argc<2) {
        fprintf(stderr,"%s takes 2 args.",argv[0]);
        return EXIT_FAILURE;
    }
    //start curses mode
    initscr();
    ...
    while ((c=getch())!='q') {
        switch (c) {
            ...
            case KEY_UP:case 'w':
                ...
                break;
            case KEY_BACKSPACE:case 'b':case KEY_LEFT:
                if (level>0) {
                    levels[level--]=NULL;
                    curr=levels[level];
                ...
                break;
            case 10:case 32:case KEY_RIGHT://enter or space
                //move into
                curr_item = current_item(m);
                exit_null(curr_item,"current_item");
                int index=item_index(curr_item);
                if (index==ERR) {endwin();perror("item_ined");exit(EXIT_FAILURE);}
